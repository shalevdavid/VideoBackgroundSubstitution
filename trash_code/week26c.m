...
mov = aviread('V:\23a.avi',100:250);                                   %%%%  loading a movie
...
%         traced_mask_REDcorr = ControlTracedMask( SetOfPoints , SetOfNewPointsREDcorr , mask_p , 5 , 2 );
        ...
        if (  sum(abs(mask(:)))>0  && sum(abs(mask_p(:)))>0  &&  sum(abs(mask_p_p(:)))>0  )
            ...
            mask_p = fill_holes_param(  (mask_p + mask_p_p.*mask + mask_p_p_centered.*mask_centered)>0  , 2500 );
        end;
...
movie2avi(FINAL,'MoviesResults\week27_3.avi','fps',movinfo.FramesPerSecond);
