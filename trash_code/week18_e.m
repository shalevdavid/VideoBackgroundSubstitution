...
for i=srf:erf-1
    ...
    RR = REDn-SourceBackgroundRED;
    GG = GREENn-SourceBackgroundGREEN;
    BB = BLUEn-SourceBackgroundBLUE;
    SS = Sn-SourceBackgroundS;
    ...
    se = strel('disk',20);
    predection = ( medfilt2( (imerode(maskp,se)+imerode(maskn,se)  )>0 , [15 15] ) + medfilt2(diffcorr,[5 5]) + diffcorr ) > 0 ;
...
movie2avi(FINAL,'MoviesResults\Final18b_try_e1.avi','fps',movinfo.FramesPerSecond);
