...
M=3;         %%%  2M+1 is the block size in pixels of the TraceObjects function
...
for i=srf:erf-1                         % if maskp is in use it should start from  srf-1
    ...
    YCBCRrunningBackground = rgb2ycbcr(runningBackground);
    diffYCBCR = abs(im2double(YCBCR)-YCBCRrunningBackground);
    diffY = clean_noise_param( (diffYCBCR(:,:,1)>0.045) , 400);
    mask13 = medfilt2( solidify_image_param( (medfilt2(diffY,[5 5]) + diffY )>0 ,5,5,15) , [5 5] ); 
...
%     [ControledSetS] = ControledPoints(SetOfNewPointsS, 2 , 4);
    [ControledSetREDcorr] =  ControlTracedMask( SetOfPoints , SetOfNewPointsREDcorr , traced_mask_REDcorr , 5 , 2 );
...
%    
    ...
    mask = solidify_image_param(  (BlockmaskASD+BlockmaskMAD+mask1+mask12+mask13)>0 , 3,3,10);
    ...
    if (iterations>=3)  
        ...
        FINAL(iterations-2) = im2frame( im2double(Ip).*repmat(mask_p,[1 1 3]) + I_Background_global.*repmat(1-mask_p,[1 1 3]) );
    end;
...
movie2avi(FINAL,'MoviesResults\week26_2.avi','fps',movinfo.FramesPerSecond);
