...
%     maskGRAY2 = CreateMask( GRAY , SourceBackgroundGRAY , maxnoiseGRAY , J , K , M );
    maskRED2 = medfilt2( CreateMaskCorr( RED , SourceBackgroundRED , 0.98 , J , K , M ) , [10 10] );
    maskGREEN2 = medfilt2( CreateMaskCorr( GREEN , SourceBackgroundGREEN , 0.98 , J , K , M ) , [10 10] );
    maskBLUE2 = medfilt2( CreateMaskCorr( BLUE , SourceBackgroundBLUE , 0.98 , J , K , M ) , [10 10] );
    
%     maskH2 = CreateMaskCorr( H , SourceBackgroundH , 0.6 , J , K , M );
%     maskS2 = CreateMaskCorr( S , SourceBackgroundS , 0.6 , J , K , M );
    
%     maskHS1 = clean_noise_param ( (maskH1+maskS1)>0 , 800);
%     maskHS2 = clean_noise_param( (maskH2 + maskS2)>0 , 800);

    maskRED2 = clean_noise_param(maskRED2,400);
    maskGREEN2 = clean_noise_param(maskGREEN2,400);
    maskBLUE2 = clean_noise_param(maskBLUE2,400);
    
    maskRGB = (maskRED2+maskGREEN2+maskBLUE2)>1;   

%     mask = (maskHS1+maskHS2+maskRGB)>0;
    
    mask = fill_shape4(maskRGB);
...
movie2avi(FINAL,'MoviesResults\Final_Idea_corr5.avi') ;
