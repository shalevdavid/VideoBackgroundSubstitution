...
%     mask2 = fill_shape_param4( maskControl ,5,5,15);
    ...
    [BlockmaskMADRED] = CreateMaskMAD( RED , SourceBackgroundRED , 0.65 , 3 );
    BlockmaskMADRED = clean_noise_param(BlockmaskMADRED,400);
    BlockmaskMADRED =  medfilt2( solidify_image_param( (medfilt2(BlockmaskMADRED,[5 5]) + BlockmaskMADRED )>0 ,5,5,15) , [5 5] );
    [BlockmaskASDRED] = CreateMaskASD( RED , SourceBackgroundRED , 0.6 , 3 );       %ASD is more robust against noise
    BlockmaskASDRED = clean_noise_param(BlockmaskASDRED,400);
    BlockmaskASDRED =  medfilt2( solidify_image_param( (medfilt2(BlockmaskASDRED,[5 5]) + BlockmaskASDRED )>0 ,5,5,15) , [5 5] );
    
    mask = solidify_image_param(  (BlockmaskASD+BlockmaskMAD+mask1)>0 , 3,3,10);
    
    FINAL(i-srf+1) = im2frame( im2double(I).*repmat(mask1,[1 1 3]) + I_Background_global.*repmat(1-mask1,[1 1 3]) );
...
movie2avi(FINAL,'MoviesResults\week25b.avi','fps',movinfo.FramesPerSecond);
