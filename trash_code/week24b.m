...
%     maskSetOfNewPointsAvg = zeros(J,K);
%     for j=1:size(SetOfNewPointsAvg,1)
%         maskSetOfNewPointsAvg(  SetOfNewPointsAvg(j,1)  ,  SetOfNewPointsAvg(j,2)  )=1;
%     end;

    [Blockmask] = CreateMaskMAD( RED , SourceBackgroundRED , 0.25 , 3 );
    
    mask = fill_shape_param4( (mask1+mask2) >0 , 3,3,15 );      %not a good idea
    
    FINAL(i-srf) = im2frame( im2double(I).*repmat(mask1,[1 1 3]) + I_Background_global.*repmat(1-mask1,[1 1 3]) );
...
movie2avi(FINAL,'MoviesResults\week24b.avi','fps',movinfo.FramesPerSecond);
