...
%     maskGRAY1 = medfilt2(abs(GRAY - SourceBackgroundGRAY)>5*maxnoiseGRAY,[5 5]);
    maskRED1 = medfilt2(abs(RED - SourceBackgroundRED)>min(5*maxnoiseRED,0.07),[5 5]);
    maskGREEN1 = medfilt2(abs(GREEN - SourceBackgroundGREEN)>min(5*maxnoiseGREEN,0.07),[5 5]);
    maskBLUE1 = medfilt2(abs(BLUE - SourceBackgroundBLUE)>min(5*maxnoiseBLUE,0.07),[5 5]);
...
movie2avi(FINAL,'MoviesResults\InterlaceM=6+NOgray4.avi') ;
