...
erf=160;                                                   % erf = ending running frame - stage 3
...
for i=srf:erf-1
    ...
    mask = fill_shape_param4( (medfilt2(mask2,[5 5]) + mask2 + medfilt2(diffcorr,[5 5]) + diffcorr)>0 ,2,2,15);
    
    if (i==srf)
        maskp = mask;
        continue;
    end;
    mask_arr(:,:,i-srf+1) = mask;
...
% movie2avi(FINAL,'MoviesResults\Final18b_try_4.avi','fps',movinfo.FramesPerSecond);
