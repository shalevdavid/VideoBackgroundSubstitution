...
%     mask_matchS = MaskMatchingSAD( mask123 , S , SourceBackgroundS , 0.2 , 3);
...
%    mask = imfill(medfilt2(MaskMatching( mask , GREEN , SourceBackgroundGREEN , 0.995 , 3) , [5 5] ) , 'holes');
%    mask = imfill(medfilt2(MaskMatching( mask , GREEN ,SourceBackgroundGREEN , 0.995 , 3) , [5 5] ) , 'holes');
...
% 
%     imshow( ( medfilt2( e.*mask+ medfilt2( imfill( medfilt2(medfilt2(e.*mask,[10 10])+e.*mask, [5 5])+e.*mask) , [10 10]) , [3 3])>0).*GREEN)
...
movie2avi(FINAL,'MoviesResults\Final17_2.avi') ;
