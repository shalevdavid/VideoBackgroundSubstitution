...
%%% Ip = Previous frame image (in double)
...
function [SetOfNewPoints,AvgComputation] = TraceObjectES(SetOfPoints, Ip, In, maskp, M, p)
...
for i=1:size(SetOfPoints,1)
    ...
    [motionVect, EScomputations] = BlockTraceES(In, Ip, maskBlock, 2*M+1, p, r1, c1);
    
    TotalComputation = TotalComputation + EScomputations;
