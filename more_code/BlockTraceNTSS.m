...
function [motionVect, NTSScomputations] =BlockTraceNTSS(imgP, imgI, maskBlock, mbSize, p, i, j)
...
costs(2,2) = costFuncMAD(imgI(i:i+mbSize-1,j:j+mbSize-1).*maskBlock, ...
                            imgP(i:i+mbSize-1,j:j+mbSize-1).*maskBlock,mbSize);
stepSize = stepMax; 
...
for m = -stepSize : stepSize : stepSize        
    for n = -stepSize : stepSize : stepSize
        ...
        costs(costRow, costCol ) = costFuncMAD(imgI(i:i+mbSize-1,j:j+mbSize-1).*maskBlock, ...
        imgP(refBlkVer:refBlkVer+mbSize-1, refBlkHor:refBlkHor+mbSize-1).*maskBlock, mbSize);
        computations = computations + 1;
    end
end
